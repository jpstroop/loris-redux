...
def create_large_image():
    ...
    image = Image.new(mode="RGB", size=(6000, 8000))
...
def create_small_image():
    # start with a black image
    image = Image.new(mode="RGB", size=(600, 800))
    draw = ImageDraw.Draw(image)
    # TOP LEFT QUADRANT
    draw.rectangle((0, 0, 299, 399), fill=GREEN)
    # TOP_RIGHT_QUADRANT
    draw.rectangle((300, 0, 599, 399), fill=RED)
    # BOTTOM_LEFT_QUADRANT
    draw.rectangle((0, 400, 299, 799), fill=BLUE)
    # BOTTOM_RIGHT_QUADRANT
    draw.rectangle((300, 400, 599, 799), fill=ORANGE)
    # RANDOM_RECTANGLE_1
    draw.rectangle((200, 500, 499, 699), fill=PURPLE)
    # RANDOM_RECTANGLE_2
    draw.rectangle((100, 150, 249, 449), fill=DARK_SLATE)
    return image


class TestImageFeatures(unittest.TestCase):
    # This just tests the large image
    def setUp(self):
        self.image = create_large_image()
...
if __name__ == "__main__":
    ...
    # create_large_image().show(command='/usr/bin/eog')
    # create_large_image().save('region_test.jpg')
    # create_large_image().save('region_test.tif')
    create_small_image().save("pillow_transcoder_tests.bmp", quality=100)

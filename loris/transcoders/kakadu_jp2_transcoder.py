...
class KakaduJp2Transcoder(AbstractJp2Transcoder, AbstractTranscoder):
    ...
    def _find_kakadu():  # THIS IS ONLY MEANT FOR TESTS. SUPPLY YOUR OWN!
        sys = system().lower()
        proc = processor()  # this may need be made more specifc
        project_dir = dirname(dirname(dirname(abspath(__file__))))
        kdu_dir = join(project_dir, "tests", "kakadu", sys, proc)
        if sys in ("linux", "darwin"):
            return (kdu_dir, join(kdu_dir, KDU_BIN))
        else:
            msg = f"Kakadu binaries not included for {sys}/{proc}"
